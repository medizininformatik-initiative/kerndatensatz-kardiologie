Profile: MII_PR_Kardio_Condition_Artherosklerotisches_Erstereignis
Id: mii-pr-kardio-condition-artherosklerotisches-erstereignis
Parent: MII_PR_Diagnose_Condition

Title: "MII_PR_Kardio_Diagnose_Artherosklerotisches_Erstereignis"
Description: "Profil zur Erfassung des Datums eines artherosklerotischen Erstereignisses im Kontext von ACRIBiS."

// code code um anzugeben, welches ereignis das erste war: 
// Koronare Herzerkrankung, TIA, Amaurosis Fugax, Apoplex, Retinainfarkt, oder Operation der Carotiden, pAVK, Bauchaortenaneurysma.
// datum als value

// valueset für codes angeben

* extension[Feststellungsdatum] 1..1 MS